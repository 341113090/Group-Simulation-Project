 ...
 * 
 * <p>
 * This is effectively a one-line text box that should span the width of the
 * whole world (although
 * the width can be changed if desired). It includes methods for managing and
 * updating scores.
 * </p>
 * 
 * @author Jordan Cohen
...
public class SuperDisplayLabel extends Actor {
    ...
    public SuperDisplayLabel(Color backColor, Color foreColor, Font font, int width) {
        this(width, font);
    ...
    public void update(String[] labels, int[] newValues) {
        this.labels = labels;
        // Arrays should be the same size, but just in case, only loop to the lower
        // length to avoid crashing
    ...
    /**
     * Takes a String and displays it centered to the screen. Note this gets called
     * bu the other
     * update() method.
     * 
     ...
     *                 sparingly, as the recalculation is fairly demanding on the
     *                 CPU.
     */
    public void update(String output, boolean recenter) {
        ...
        image.drawString(output, centeredX, 22);
     ...
     * <p>
     * <b>IMPORTANT:</b> Set your Font in your GreenfootImage before you send it
     * here.
     * </p>
     * <p>
     * Use this instead of Greenfoot.drawString to center your text, or just call
     * getStringWidth
     * directly and draw it yourself if you prefer the control over the ease of use.
     * </p>
     * 
     * @param canvas  The GreenfootImage that you want to draw onto, often the
     *                background of a World, but
     ...
     * @param bottomY the y Coordinate at the baseline of the text (similar to
     *                GreenfootImage.drawString)
     * 
     * @since June 2021
     */
     ...
     * </p>
     * <p>
     * Similar to the method above, except it always centers the text on the whole
     * image
     * instead of a specified x position. UNTESTED!
     * </p>
     * 
     * @param canvas  The GreenfootImage that you want to draw onto, often the
     *                background of a World, but
     ...
     * 
     * <p>
     * Get the Width of a String, if it was printed out using the drawString command
     * in a particular
     * Font.
     * </p>
     * <p>
     * There is a performance cost to this, although it is more significant on the
     * Gallery, and
     * especially on the Gallery when browsed on a mobile device. It is appropriate
     * to call this in the
     * constructor, and in most cases it is ideal NOT to call it from an act method,
     * especially
     * every act.
     * </p>
     * 
     * <p>
     * In cases where values are pre-determined, it may be ideal to cache the values
     * (save them) so
     * you don't have to run this repeatedly on the same text. If you do this in the
     * World constructor,
     * there is no performance cost while running.
     * </p>
     * 
     * <h3>Performance & Compatibility:</h3>
     * <ul>
     * <li>Locally, performance should be sufficient on any moderate computer
     * (average call 0.1-0.2ms on my laptop)</li>
     * <li>To be compatible with Greenfoot Gallery, removed use of getAwtImage() and
     * replaced with getColorAt() on a GreenfootImage</li>
     * <li>On Gallery, performance is about 10x slower than locally (4ms on Gallery
     * via Computer). For reference, an act() should be
     * less than 16.6ms to maintain 60 frames/acts per second.</li>
     * <li>HUGE performance drop on Gallery via Mobile devices - not sure why, going
     * to ignore for now. (Average update duration 34ms, more
     ...
     * 
     * @since June 2021
     * @version December 2021 - Even more Efficiency Improvement - sub 0.06ms per
     *          update on setSpeed(100)!
     */
        ...
        while (running) {
            ...
            for (int i = fontSize - 1; i >= 0 && !found; i -= marginOfError) {
                ...
                if (temp.getColorAt(checkX, i).getAlpha() != 0) {
                    // This lets me only look at every other pixel on the first run - check back one
                    // to the right
                    // when I find a pixel to see if I passed the first pixel or not. This should
                    // almost half the
            ...
            checkX -= 3; // shift 3 pixels at a time in my search - above code will make sure I don't
                         // miss anything
